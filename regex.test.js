...
function capitalA(s){
    let p1 = /[A-Z]/, p2=/[a-z]/, p3=/A$/, p4=/\d/, p5=/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/
    if(p1.test(s) && p2.test(s) && p3.test(s) && p4.test(s)==false && p5.test(s)==false)    
    return true;
    return false;     
}
//
...
function ioEmail(email){
    let pattern = /.io$/, p1=/[@]/g, p2=/[.]/g
    if(email.match(p1).length==1 && email.match(p2).length==1 && pattern.test(email))
    return true;
    return false;        
}
...
function imagesSearcher(text){
    let pattern = /(\b[\w-]+\.(jpg|jpeg|png)\b)/gi;
